...
exports.createOrder = async (req, res) => {
    try {
        ...
        if (!user || !products || products.length === 0 || !totalAmount) {
            return res.status(400).json({ message: 'Por favor, preencha todos os campos obrigatórios e adicione produtos.' });
        }
        ...
        // Verificar e atualizar o estoque dos produtos
        for (const item of products) {
            ...
            if (product.quantity < item.quantity) {
                return res.status(400).json({ message: `Estoque insuficiente para o produto: ${product.name}. Disponível: ${product.quantity}` });
            }
...
exports.getOrdersByUser = async (req, res) => {
    try {
        ...
        console.error(error);
        res.status(500).json({ message: 'Erro ao buscar pedidos por usuário', error: error.message });
    }
};